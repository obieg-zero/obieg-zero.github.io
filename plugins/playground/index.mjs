...
var PALETTE = [
  { type: "upload", label: "Upload", icon: upload_default, config: { docGroup: "" } },
  { type: "classify", label: "Classify", icon: tag_default, config: { labels: "", minScore: "0.3", model: "Xenova/multilingual-e5-small", language: "pol" } },
  { type: "embed", label: "Embed", icon: layers_default, config: { model: "Xenova/multilingual-e5-small", chunkSize: "200", language: "pol", docGroup: "" } },
  { type: "filter", label: "Filter", icon: filter_default3, config: { contains: "", pages: "" } },
  { type: "extract", label: "Extract", icon: cpu_default, config: { questions: "", topK: "2", modelUrl: BIELIK } },
  { type: "extract-api", label: "ExtractAPI", icon: globe_default, config: { questions: "", topK: "2", apiUrl: "https://api.openai.com/v1/chat/completions", apiKey: "", apiModel: "gpt-4o-mini" } },
...
function SettingField({ label, settingKey }) {
  ...
  return /* @__PURE__ */ jsx(Field, { label: /* @__PURE__ */ jsxs(Fragment2, { children: [
    ...
    "input",
    {
      ...
      value,
      placeholder: isPassword ? "sk-..." : settingKey,
    ...
    project && /* @__PURE__ */ jsx(Box, { grow: true, header: /* @__PURE__ */ jsx(Cell, { label: true, children: "Schemat" }), body: /* @__PURE__ */ jsxs(Fragment2, { children: [
      ...
      s.leftTab === "templates" ? /* @__PURE__ */ jsx(Fragment2, { children: s.templates.map((t) => /* @__PURE__ */ jsx(
        ListItem,
        {
          label: t.name,
          separator: true,
          detail: t.nodes.filter((n) => n.type === "upload").map((n) => n.data?.config?.docGroup || n.data?.label).join(" + "),
          onClick: () => {
            a.loadTemplate(t.id);
...
function RightPanel() {
  ...
  if (!node || !node.data.config || !Object.keys(node.data.config).length) return null;
  const cfg = Object.entries(node.data.config);
  const settings = Object.entries(node.data.settings || {});
  const onCfg = (k, v) => {
    a.setNodes((ns) => ns.map((n) => n.id === node.id ? { ...n, data: { ...n.data, config: { ...n.data.config, [k]: v } } } : n));
  };
  const onLabel = (v) => {
...
function PlaygroundProvider({ children: children2 }) {
  ...
  useEffect(() => {
    ...
    host.db.getPipelineByProject(project).then((p) => {
  ...
  function loadTemplate(id2) {
    ...
    if (!t || !project) return;
    setNodes(t.nodes.map((n) => ({ ...n, data: { ...n.data, config: { ...n.data.config || {} }, settings: n.data.settings ? { ...n.data.settings } : void 0 } })));
    setEdges([...t.edges]);
    setSelectedId(null);
    setTimeout(() => rfInstance.current?.fitView({ padding: 0.2 }), 50);
  }
  function onDrop(e) {
    ...
    const p = PALETTE.find((p2) => p2.type === type);
    if (!p) return;
    const id2 = `${type}-${Date.now()}`;
    setNodes((n) => [...n, { id: id2, type, position: rfInstance.current.screenToFlowPosition({ x: e.clientX, y: e.clientY }), data: { label: p.label, config: { ...p.config } } }]);
    setSelectedId(id2);
  }
  function setNodeResult(id2, status, result) {
    setNodes((ns) => ns.map((n) => n.id === id2 ? { ...n, data: { ...n.data, status, result: result || n.data.result } } : n));
  }
  function addDataNodes(pid, items, posRef) {
    const right = dataNodeSide.current++ % 2 === 0;
    const px = posRef ? posRef.x + (right ? 280 : -240) : 500, py = posRef ? posRef.y - (items.length - 1) * 14 : 0;
    setNodes((n) => [...n, ...items.map((it, i) => ({ id: it.id, type: "data", position: { x: px, y: py + i * 28 }, data: { label: it.label, detail: it.detail } }))]);
  ...
  function addGraphNodes(_pid, graph, posRef) {
    const baseX = posRef ? posRef.x : 500, baseY = posRef ? posRef.y : 0;
    const docs = graph.nodes.filter((n) => n.type === "document");
    const values = graph.nodes.filter((n) => n.type !== "document");
    const sharedIds = /* @__PURE__ */ new Set();
    for (const v of values) {
      const sources = new Set(graph.edges.filter((e) => e.to === v.id).map((e) => e.from));
      if (sources.size > 1) sharedIds.add(v.id);
    }
    setNodes((n) => [
      ...n,
      ...docs.map((d, i) => ({ id: `viz:${d.id}`, type: "doc", position: { x: baseX - 200, y: baseY + 80 + i * 40 }, data: { label: d.label } })),
      ...values.map((v, i) => ({ id: `viz:${v.id}`, type: "entity", position: { x: baseX + 280, y: baseY + 80 + i * 36 }, data: { label: v.label, detail: v.type, shared: sharedIds.has(v.id) } }))
    ]);
    setEdges((e) => [...e, ...graph.edges.map((ge) => ({
      id: `viz:${ge.id}`,
      source: `viz:${ge.from}`,
      sourceHandle: "right",
      target: `viz:${ge.to}`,
      targetHandle: "left",
      label: ge.label,
      style: { strokeWidth: 1, stroke: sharedIds.has(ge.to) ? "var(--color-primary)" : "rgba(255,255,255,0.2)" },
  ...
  async function runPipeline() {
    ...
    const pipeEdges = edges.filter((e) => pipeEdgeIds.has(e.source) && pipeEdgeIds.has(e.target));
    ...
    await execPipeline(host, project, pipeNodes, pipeEdges, [], addLog, (id2, status, result, meta) => {
      ...
      if (meta.files) addDataNodes(id2, meta.files.map((f) => ({ id: `viz:${id2}:${f.name}`, label: f.name })), pos);
      if (meta.classified) addDataNodes(id2, [
        ...Object.entries(meta.classified).filter(([, ids]) => ids.length).map(([label, ids]) => ({ id: `viz:${id2}:${label}`, label: `${label} (${ids.length})` })),
        ...meta.rejected?.length ? [{ id: `viz:${id2}:rejected`, label: `odrzucone (${meta.rejected.length})` }] : []
      ], pos);
  ...
  function exportPipeline() {
    ...
    a2.download = `${project}.pipeline.json`;
    a2.click();
...
function CenterCanvas() {
  ...
  return /* @__PURE__ */ jsx(Fragment2, { children: !project ? /* @__PURE__ */ jsx("div", { className: "hero flex-1", children: /* @__PURE__ */ jsx("div", { className: "hero-content text-center", children: /* @__PURE__ */ jsxs("div", { className: "max-w-lg space-y-5", children: [
    ...
    index,
    {
      ...
      onNodeClick: (_, node) => {
        if (node.data.config) a.setSelectedId(node.id);
      },
